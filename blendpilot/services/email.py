"""
BlendPilot AI — Email Review Service
...
class ReviewEmailContent(BaseModel):
    ...
    asset_name: str = Field(..., description="Name of the asset being reviewed")
    version: str = Field(default="1.0", description="Asset version string")
    preview_image_path: str = Field(..., description="Path to the preview render")
    ...
    design_spec_summary: str = Field(default="", description="Original design requirements")
...
class FeedbackItem(BaseModel):
    ...
    target: str = Field(..., description="Object or area the feedback applies to")
...
class EmailService:
    ...
    async def send_review_email(
        ...
        """
        logger.info("Preparing review email to %s for asset '%s'", to, content.asset_name)

        # Phase 9: implement actual SMTP sending
        # For now, log the intent and return a stub
        logger.warning("Email sending not yet implemented — returning stub result.")
    ...
    async def parse_feedback(self, email_body: str, sender: str = "") -> ParsedFeedback:
        ...
        """
        logger.info("Parsing feedback email from '%s' (%d chars)", sender, len(email_body))

        # Phase 9: implement actual LLM-based parsing
        # For now, return empty feedback
        logger.warning("Feedback parsing not yet implemented — returning empty result.")
    ...
    def compose_review_html(self, content: ReviewEmailContent) -> str:
        ...
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h1>BlendPilot AI — Asset Review</h1>
            ...
            <p style="color: #888; font-size: 12px;">
                Generated by BlendPilot AI — {content.version}
            </p>
        </body>
        </html>
        """
